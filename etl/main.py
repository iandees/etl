...
class Urllib2ExtractionTask(ExtractionTask):
    ...
    def extract(self, source_key, source_urls):
        ...
        for source_url in source_urls:
            ...
            file_path = os.path.join(download_path, resp.geturl().split('/')[-1])
...
class EsriRestExtractionTask(ExtractionTask):
    ...
    def extract(self, source_key, source_urls):
        ...
        for source_url in source_urls:
            ...
            source_downloaded = os.path.join(download_path, parts.netloc + '.json')
            with open(source_downloaded, 'w') as f:
                ...
                f.write('\n]\n}\n')
            self.logger.info("Downloaded %s ESRI features for file %s", size, source_downloaded)
            output_files.append(source_downloaded)
        return output_files
